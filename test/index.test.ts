...
describe("recipe parser eng", () => {
  it("returns an object", () => {
    expect(typeof parse("1 tasse d'eau", "fra")).to.equal("object");
  });

  describe("translates the quantity", () => {
    it(" sans à -> 1 cuillière à café d'eau", () => {
      expect(parse("1 cuillière à café d'eau", "fra").quantity).to.equal(1);
    });
    it(" sans à -> 1,5 cuillière à café d'eau", () => {
      expect(parse("1,5 cuillière à café d'eau", "fra").quantity).to.equal(1.5);
    });
    it(" sans à -> 1 1/2 cuillière à café d'eau", () => {
      expect(parse("1 1/2 cuillière à café d'eau", "fra").quantity).to.equal(
        1.5
      );
    });
    it(" sans à -> 1/3 cuillière à café d'eau", () => {
      expect(parse("1/3 cuillière à café d'eau", "fra").quantity).to.equal(
        0.333
      );
    });
    it(" sans à -> environ 1/2 cuillière à café d'eau", () => {
      expect(
        parse("about 1/2 cuillière à café d'eau", "fra").quantity
      ).to.equal(0.5);
    });
